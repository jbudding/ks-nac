...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Create a simple Access-Request packet with wrong credentials
    let mut packet = Vec::new();

    // RADIUS Header: Code=1 (Access-Request), ID=2, Length=TBD, Authenticator=16 zeros
    packet.push(1u8); // Code: Access-Request
    packet.push(2u8); // Identifier
    ...
    // User-Name attribute (type=1) - invalid user
    ...
    // User-Password attribute (type=2) - wrong password
    ...
    println!("Sending RADIUS Access-Request with invalid credentials...");
    ...
    match socket.recv_from(&mut buf) {
        Ok((_len, _addr)) => {
            ...
            match response_code {
                ...
                3 => println!("✗ Authentication failed! (Access-Reject) - Expected result"),
